from platform_core.knowledge_rag.retrieval.vector_search import vector_search
from platform_core.knowledge_rag.retrieval.bm25_search import BM25Index
from platform_core.knowledge_rag.retrieval.self_query import rewrite_query

def hybrid_search(query: str, document_id: str, top_k: int = 10, retrieval_method: str = "hybrid") -> list:
...
def search_with_self_query(user_question: str, document_id: str, top_k: int = 10) -> list:
    """
    Rewrites the question via Gemini, runs hybrid search, and boosts scores of preferred chunk types.
    """
    ...
    if preferred_type != "any":
        for result in results:
            if result.get("chunk_type") == preferred_type:
                result["score"] *= 1.25
